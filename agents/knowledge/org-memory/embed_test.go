...
func TestFakeEmbedderSimilarity(t *testing.T) {
	...
	pay1, _ := e.Embed(ctx, "money must be stored as int64 cents", RoleDocument)
	pay2, _ := e.Embed(ctx, "store money in int64 cents never float", RoleDocument)
	css, _ := e.Embed(ctx, "css flexbox layout for the sidebar", RoleDocument)
