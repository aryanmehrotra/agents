...
> **Correction and measured limitation (2026-07-30).** As deployed until this date, the above described an *intended* design that the implementation did not realise. The prior was computed as a weighted **sum** clamped at `prior_cap`; since recency, importance and retrievability alone always exceed a 0.01 cap, the sum saturated and **every** candidate received exactly `+prior_cap`. A constant offset cannot reorder anything, so authority — and every other prior — was inert, and any validation of the "bounded prior behaves safely" property was vacuous rather than passing. The implementation now normalises the priors to a weighted mean of $[0,1]$ signals before scaling by `prior_cap` (score normalisation prior to linear combination: Lee 1997; Montague & Aslam 2001), which is bounded by construction rather than by clipping.
>
> The measured consequence should be stated plainly, because it qualifies this subsection's claim: at the deployed weights authority's maximum contribution is $\text{prior\_cap} \times w_{\text{auth}}/\sum w = 0.01 \times 0.15/1.35 \approx 0.0011$, against a **median adjacent cosine gap of 0.011** in top-5 results. Toggling $w_{\text{auth}}$ between 0 and 0.15 across 20 queries changes **no** ordering. Authority is therefore correctly bounded and, at present, *practically inert* — a formality rather than an operative signal. Raising `prior_cap` to ≈0.05 would make it operative, at a proportional increase in the burial risk the cap exists to prevent. **We have kept `prior_cap` at 0.01 and accordingly do not claim authority weighting as an operative ranking signal in this system**: relevance is effectively absolute, the authority mechanism is implemented and bounded but does not change retrieved sets at these weights, and describing it otherwise would overstate what the deployment does. All ranking results elsewhere in this paper were produced under the inert-prior implementation and are therefore effectively **pure-cosine** rankings; this does not invalidate them, but they should not be read as evidence about prior behaviour.
...
### 7.2 Executed retrieval + abstention evaluation (dense vs. BM25)

We ran a first executed evaluation of the two properties the thesis turns on: retrieval quality on answerable queries, and abstention on unanswerable ones. **Labeled set:** 60 answerable queries — LLM-paraphrased questions, each targeting the single source decision it was written from (a proxy for real user queries, not human-gold) — and 20 unanswerable off-topic queries (geography, cooking, sport) for which the corpus genuinely holds nothing, so *returning nothing* is the correct behaviour. **Systems:** the live dense system with its calibrated floor, vs. a BM25 lexical baseline (Okapi, k1=1.5, b=0.75) over the same 1,643-decision corpus. Metrics are Recall@k and MRR@10 on answerable queries and abstention accuracy (fraction correctly returning nothing) on unanswerable queries.

| System | Recall@1 | Recall@3 | MRR@10 | Abstained-on-answerable | Abstention acc. (unanswerable) |
|---|---|---|---|---|---|
| Dense (org-memory, with floor) | 0.65 | 0.82 | 0.736 | 0.00 | **1.00** |
| BM25 (lexical, no floor) | 0.68 | 0.77 | 0.731 | — | **0.25** |
...
### 7.4 A second pilot (v2): necessity partition + a "surfacing ≠ compliance" result

Rebuilt to the v1 red-team's specification: 26 **org-idiosyncratic** decisions a base model cannot know a priori (mined from internal-architecture decisions, textbook practices excluded); a deliberately **junior** control (a small model, `haiku`) as the least-knowledgeable baseline; objective token-based checks; and a strict, anonymized, evidence-quoting judge. (A first v2 pass was discarded when a verbatim internal env-var in a "no-memory" answer exposed a file-hygiene leak — the control had seen the answer key; it was re-run with a blind task-only file.)

**Necessity partition (the self-learning signal).** The junior's no-memory default violated the org decision in **17/26 (65%)** of cases and already complied in 9/26 — a *measured* fraction of the corpus that is genuinely necessary (high prediction error) vs. redundant with model priors. This is the identifiable quantity the design learns from, and it is precisely reward-prediction-error / predictive-coding residual (Schultz et al. 1997; Rao & Ballard 1999): encode what the base model gets wrong, explain away what it gets right.

**Uplift on the necessary subset (N=17), the result:**

| arm | compliance | Wilson 95% |
|---|---|---|
| control (junior, no memory) | 0/17 = 0% | — |
...
With retrieval, agent, and decisions held fixed, changing only the injection format moved compliance from **0% → 85%**, closing most of the distance to the oracle ceiling. This confirms that the deployment gap was the *presentation of surfaced context to a weak agent*, not retrieval or content — an actionable, and now-shipped, design result. A follow-up (v2c) isolated *which* part of the fix carries the effect: single-pass authoritative injection with **no** retry already reaches 82–88% (independent cross-family judge, 100% inter-judge agreement), and adding a deterministic check-and-retry moves the semantic score by **+0%** — the **injection format is the lever; the retry is not**. (The retry did lift a token-based mechanical check, but that gain did not survive an independent semantic judge — a Goodhart artifact of using a gameable proxy to drive the loop; a genuinely rigorous executable check would require code execution.) (Caveats: small N with wide intervals; a single anonymized evidence-quoting LLM judge; the retry is model self-report — an *executable* check-and-retry loop is the stronger form; a directional pilot, not a powered study.)
