...
var registry = []specialist{
	{"research", "research-agent", "http://localhost:8008", "research", []string{"question"},
		...
		[]string{"http://", "https://"}},
	{"breaking", "breaking-change-agent", "http://localhost:8019", "breaking-changes", []string{"title", "diff"},
		"detect breaking API/contract changes in a diff before merge (removed symbols, new required fields/params, type changes)",
		[]string{"breaking change", "breaking changes", "breaking api", "api break", "contract break", "backwards compatible", "backward compatible", "backwards compatibility"}},
	...
	{"testgen", "test-gen-agent", "http://localhost:8017", "testgen", []string{"code"},
		"write unit tests for a piece of code (for Go, compile and run them to verify they pass)",
		[]string{"write tests", "write a test", "unit test", "unit tests", "test coverage", "generate tests", "add tests for"}},
	{"flaky", "flaky-test-agent", "http://localhost:8018", "flaky", []string{"text"},
		"find flaky tests in CI run history — tests that both pass and fail across runs — and recommend quarantine",
		[]string{"flaky", "flaky test", "flaky tests", "quarantine", "intermittent test", "ci history", "flaky ci"}},
